  ...
  return (
    <>
      ...
      <div className="metrics-grid">
        {[
          { label: 'Pendientes',  value: 0, icon: '⏳', color: 'rgba(245,158,11,.12)' },
          { label: 'En revisión', value: 0, icon: '🔍', color: 'var(--primary-dim)'   },
          { label: 'Aprobadas',   value: 0, icon: '✅', color: 'rgba(34,197,94,.12)'  },
          { label: 'Incompletas', value: 0, icon: '⚠️', color: 'rgba(239,68,68,.12)'  },
        ].map(m => (
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
                    {[...Array(6)].map((_, i) => (
                      ...
                      }} onClick={() => showToast('Ver foto completa')}>📷</div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
                    <button className="btn-primary" onClick={() => showToast('Evidencia aprobada ✓')}>✓ Aprobar</button>
                    <button className="btn-secondary" onClick={() => showToast('Marcada como incompleta')}>Marcar incompleta</button>
                    <button className="btn-secondary" onClick={() => showToast('Solicitud enviada')}>Solicitar aclaración</button>
                    <button className="btn-danger" onClick={() => showToast('Evidencia rechazada')}>Rechazar</button>
                  </div>
                </div>
              ) : (
                <div className="card">
